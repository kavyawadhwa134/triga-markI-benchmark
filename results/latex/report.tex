...
\begin{tabular}{@{}ll@{}}
\toprule
\textbf{GeN-Foam / OFFBEAT} & 2-group diffusion, finite volume\\
\textbf{Cardinal} & OpenMC / MOOSE / NekRS --- continuous-energy Monte Carlo\\
\midrule
Nuclear data & ENDF/B-VIII.0 (both); ENDF/B-VII.1 for the Cardinal coupled run\\
Repository & \texttt{github.com/kavyawadhwa134/triga-markI-benchmark}\\
Independence & Cardinal results frozen under git tag before the GeN-Foam case was read\\
\bottomrule
\end{tabular}
\vfill
...
\begin{tabular}{@{}lll@{}}
\toprule
 & \textbf{GeN-Foam / OFFBEAT} & \textbf{Cardinal}\\
\midrule
...
\newpage
%======================================================================
\section{Results: GeN-Foam / OFFBEAT}
...
difference of $\mathbf{-4114}$\,\textbf{pcm}. This is GeN-Foam's own measurement of
...
committed files. Section~\ref{sec:comp} compares it against Cardinal's computed value.
\end{callout}
...
\begin{table}[H]\centering\small
\caption{Cardinal local fuel temperature coefficient by interval.}
\begin{tabular}{@{}lr@{}}
\toprule
...
\begin{callout}[alertbg]{The Cardinal thermal-hydraulic field is not converged}
...
4 & Diffusion vs.\ Monte Carlo & $1.331$ & $1.388100$ & $-4114$\pcm & GeN-Foam internal\\
...
Both cores are bare boxes with vacuum boundaries: the GeN-Foam \texttt{validation\_step6.csv}
records the model verbatim as \emph{``eigenvalue diffusion, bare homogenized box vacuum
BCs''}. \textbf{Geometry is therefore not the explanation.} An earlier hypothesis that GeN-Foam
...
\midrule
1. Homogenisation & Cardinal het.\ $\rightarrow$ homog. & $+3920$\pcm & Monte Carlo both sides; pure geometry smearing\\
2. Solution method & Cardinal homog.\ $\rightarrow$ GeN-Foam & $+8320$\pcm & Same homogenised bare box; pure method\\
\midrule
\rowcolor{rowbg}
Total & Cardinal het.\ $\rightarrow$ GeN-Foam & $+12566$\pcm & ---\\
\bottomrule
\end{tabular}
\end{table}

The residual $+8320$\pcm{} is corroborated independently by GeN-Foam's own smoke test
...
treatment dominating in the bare case. Cardinal's core leaks 20.1\% of its neutrons, a regime in
which few-group diffusion theory is least reliable.

A further uncontrolled variable applies to Cases 2--3: the project workflow intended Cardinal to
generate the two-group MGXS and supply it to GeN-Foam, but \textbf{that phase was not run}. GeN-Foam
condensed its own constants independently (Case 11). Re-solving the GeN-Foam diffusion model with
Cardinal-generated cross sections would isolate the remaining method error cleanly.
...
GeN-Foam's own data places the method difference at thousands of pcm on even the simplest
...
\begin{table}[H]\centering\small
\caption{Fuel temperature coefficient, compared in GeN-Foam's own operating band.}
\begin{tabular}{@{}lll@{}}
\toprule
 & \textbf{Value} & \textbf{Origin}\\
\midrule
Cardinal, 500--\SI{600}{\kelvin} & $-7.76$\,pcm/K & computed from $S(\alpha,\beta)$ ZrH data\\
Cardinal, 296--\SI{1000}{\kelvin} average & $-6.34$\,pcm/K & computed\\
GeN-Foam, transient & $-10.000$\,pcm/K & constant input; origin undocumented\\
\midrule
\rowcolor{rowbg}
Difference in the GeN-Foam band & $-29\%$ & GeN-Foam more negative\\
\bottomrule
\end{tabular}
\end{table}

\begin{callout}[alertbg]{Recommended action}
A coefficient 29\% more negative shuts a power excursion down faster. GeN-Foam's peak power of
...
Cardinal's value is traceable to a nuclear-data calculation; GeN-Foam is not traceable from the
committed files. GeN-Foam should either substitute a temperature-dependent coefficient and re-run
...
reliable range.
\item \textbf{Cardinal's coolant field is not converged} (34.6\% energy closure), so
\SI{294.2}{\kelvin} is a lower bound rather than a result.
\end{enumerate}

\subsection{Cases 10--12 --- no Cardinal counterpart}
The transient is out of Cardinal's defined scope: the project workflow specifies a steady-state
production run and defines no transient phase. This is a documented scope decision rather
than an omission, but it means GeN-Foam's transient results --- including the peak power on which
...
2. Numerical & PARTIAL & Cardinal neutronics and conduction converged; Cardinal CFD thermal field is not; no mesh or order refinement study either side\\
3. Physics & PARTIAL & Cardinal conduction within 1.1\% of analytic; ZrH feedback confirmed; but Cardinal CFD shows an unphysical sub-inlet temperature and GeN-Foam's feedback coefficient is an undocumented input\\
...
\midrule
1 & GeN-Foam feedback coefficient & Document or recompute the assumed $-10$\,pcm/K & ---\\
2 & \keff{} method gap & Generate Cardinal's two-group MGXS; re-solve the GeN-Foam diffusion with it & \SI{2}{\hour} CPU\\
3 & Cardinal CFD not converged & Extend to $\sim$5 flow-throughs after resolving the filter undershoot & \SI{5}{\hour} GPU\\
4 & Cardinal sub-inlet temperature & Reduce scalar HPFRT strength; verify with a short run & \SI{1}{\hour} GPU\\
...
Cardinal thermal-hydraulics not converged & 34.6\% energy closure after 1.17 flow-throughs;
coolant temperatures are lower bounds.\\
Unphysical minimum temperature & \SI{291.52}{\kelvin} against a \SI{293}{\kelvin} inlet with
no heat sink --- a filter-stabilisation artifact that does not decay.\\
...
The converged CFD is not yet coupled back.\\
...
Lattice constants generated independently & The intended shared MGXS was never produced, so
the two-group constants are an uncontrolled variable in the eigenvalue comparison.\\
...
Both calculations and all data are committed to
\texttt{github.com/kavyawadhwa134/triga-markI-benchmark} (branch \texttt{master}).

\subsection{GeN-Foam / OFFBEAT}
The OpenFOAM case is under \texttt{genfoam/}:
...
\midrule
\texttt{genfoam/constant/neutroRegion/nuclearData} & Two-group cross sections\\
\texttt{genfoam/triga\_steady/} & Steady-state case (\texttt{Allrun}, \texttt{Allclean})\\
\texttt{genfoam/triga\_steady/0/} & Initial and boundary fields, fluid and neutronic regions\\
\texttt{genfoam/triga\_steady/system/} & \texttt{blockMeshDict}, schemes, solution controls\\
\texttt{genfoam/triga\_transient\_rod/} & $+0.5\,\$$ insertion transient\\
\bottomrule
\end{tabular}
\end{table}

\subsection{Cardinal / OpenMC / MOOSE / NekRS}
Inputs are under \texttt{cardinal/}; full history and the independence tag are in
the Cardinal working repository (\texttt{pcm-triga-cardinal}).
...
Neutronics & \texttt{./scripts/run\_openmc.sh unitcell baseline}\\
Coupled & \texttt{./scripts/run\_coupled.sh production 8}\\
Thermal-hydraulics & \texttt{./scripts/run\_nekrs.sh 8}\\
Temperature coefficient & \texttt{cardinal/openmc/temperature\_coefficient.py}\\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
\texttt{validation\_step2.csv} & GeN-Foam solver acceptance\\
\texttt{validation\_step5.csv} & GeN-Foam lattice constants and diffusion smoke test\\
\texttt{validation\_step6.csv} & GeN-Foam core eigenvalue and transient summary\\
\texttt{transient\_rod\_power.csv} & GeN-Foam transient time history\\
\texttt{cardinal\_openmc\_unitcell.csv} & Cardinal \kinf, all configurations and seeds\\
\texttt{cardinal\_openmc\_core*.csv} & Cardinal \keff, heterogeneous and homogenised\\
\texttt{cardinal\_fuel\_temperature\_coefficient.csv} & Cardinal temperature-coefficient sweep\\
\texttt{cardinal\_coupled\_*.csv} & Cardinal coupled results per Picard iteration\\
\texttt{cardinal\_thermal\_profiles.csv} & Cardinal axial and radial profiles\\
\texttt{cardinal\_nekrs\_convergence.csv} & Cardinal CFD convergence history\\
\texttt{crosscode\_comparison.csv} & Case-by-case comparison, machine-readable\\
\bottomrule
\end{tabular}
\end{table}
...
diffusion-versus-Monte-Carlo on geometrically identical bare cores. GeN-Foam's own data
...
\item \textbf{The fuel temperature coefficient is the outstanding physics question.} Cardinal
computes $-7.76$\,pcm/K in the GeN-Foam operating band from ZrH scattering data; GeN-Foam uses a constant
...
\item \textbf{Two results are incomplete and are reported as such.} Cardinal's thermal-hydraulic
field has not converged, and Cardinal did not attempt the transient. Neither is presented as a
result.
...
\texttt{github.com/kavyawadhwa134/triga-markI-benchmark} (branch \texttt{master}).
the Cardinal full history and independence tag: the Cardinal working repository (\texttt{pcm-triga-cardinal}).
Cardinal results were frozen at tag \texttt{pcm-independent-v1} before any GeN-Foam file was read.
\end{document}
