# pcM ↔ pcL Comparison — TRIGA Mark I

**Date:** 2026-09-13
**pcM:** Cardinal / OpenMC / MOOSE / NekRS (continuous-energy Monte Carlo + FE conduction + spectral-element LES)
**pcL:** GeN-Foam / OFFBEAT (2-group diffusion + finite-volume thermal-hydraulics + fuel performance)

## Provenance and independence

| | |
|---|---|
| pcM source | this repository, results frozen at tag `pcm-independent-v1` |
| pcL source | `github.com/kavyawadhwa134/triga-markI-benchmark`, branch **`master`**, `results/validation_step{2,5,6}.csv`, `transient_rod_power.csv` |
| Retrieved | 2026-09-13 |

pcM's results were produced and tagged **before** pcL's branch was read. No pcM parameter
was adjusted after reading it. The tag makes this verifiable from git history.

> **Note on an earlier error.** pcL's results were initially reported as absent because only
...
| # | Case | pcM | pcL | Difference | Comparable? |
|---|---|---|---|---|---|
...
| 4 | Diffusion vs Monte Carlo | — | 1.331 vs 1.388100 | −4114 pcm | pcL-internal benchmark |
...
| 8 | Max coolant temperature | 294.2 K (unconverged) | 306.5 K | — | ❌ pcM not converged |
| 9 | Total power | 250 kW | 250 kW | 0 | ✅ identical by specification |
| 10 | +0.5 $ transient | not run | 489.7 kW peak | — | ❌ no pcM result |
| 11 | 2-group MGXS | not generated | full set | — | ❌ no pcM result |
| 12 | Solver acceptance | not formally recorded | SmallESFR PASS (4.9e-5) | — | ❌ no pcM result |
...
threshold**. pcM's uncertainty is ~6× tighter, reflecting a 25× larger particle budget.
...
Both models are **bare boxes with vacuum boundaries** — pcL's `validation_step6.csv` records
the core verbatim as *"eigenvalue diffusion, bare homogenized box vacuum BCs"*. Geometry is
therefore **not** the explanation, and an earlier hypothesis that pcL modelled a graphite
...
pcM's homogenised variants, all Monte Carlo, ENDF/B-VIII.0:
...
# 4. Case 4 — pcL's own method benchmark
...
This is pcL's own measurement of deterministic-vs-stochastic disagreement on the simplest
...
**Consequence:** the 500 pcm criterion cannot be applied between pcM's Monte Carlo k and
pcL's diffusion k. They are different quantities. This finding comes from pcL's own data,
not from pcM's interpretation.
...
leakage treatment dominating in the bare case: pcM's core leaks **20.1%**, which is where
...
The project workflow intended pcM to generate the 2-group MGXS and hand it to pcL (Phase 6).
**pcM did not run that phase.** pcL therefore condensed its own set:
...
further uncontrolled variable in case 2/3. Re-solving pcL's diffusion with pcM-generated
...
**pcM computed it** from first principles: fuel temperature swept with moderator held at
293 K, using `c_H_in_ZrH` and `c_Zr_in_ZrH` thermal scattering data.

| T_fuel band | pcM coefficient |
|---|---|
...
**pcL appears to have assumed it.** From `validation_step6.csv`, Doppler feedback of
...
**Comparison in pcL's own operating band (500–600 K): pcM −7.76 pcm/K vs pcL −10.000 pcm/K —
pcL is 29% more negative.**

*Why it matters:* this coefficient governs the entire transient result. A coefficient 29% too
negative shuts a power excursion down faster, so pcL's peak power (489.7 kW) is plausibly
...
**Recommended action:** pcL should either substitute pcM's temperature-dependent coefficient
...
| Quantity | pcM | pcL | Why not comparable |
|---|---|---|---|
| Power peaking | 2.2544 | 2.45 | pcM: 120 discrete pins, heterogeneous. pcL: single zone, "cosine-like, flat by construction" |
...
| Max coolant temperature | 294.2 K | 306.5 K | **pcM's CFD is not converged** — 34.6% energy closure; this is a lower bound, not a result |

Three independent reasons the fuel temperatures cannot be differenced:

1. **pcM is an average pin.** Its own peaking factor of 2.2544 implies a hot channel is
   materially hotter.
2. **pcM's film coefficient is a correlation**, not a solution — Dittus–Boelter at Re = 4755,
   which is transitional and below the correlation's reliable range. It supplies ~63% of
   pcM's total temperature rise.
3. **Different power basis.** pcM normalises to 2083.33 W per pin (250 kW / 120 pins);
   pcL's sub-scale pin carries a different share.

---

# 8. Cases 10, 12 — no pcM counterpart

| Quantity | pcL value | pcM |
|---|---|---|
...
The transient is **out of pcM's defined scope**: the project workflow specifies a
...
2. **The k-effective difference is method, not error.** It decomposes into +3920 pcm of
   ...
   cores. pcL's own smoke test independently measures a −4114 pcm method difference on the
...
4. **The fuel temperature coefficient is the most consequential open item.** pcM computes
   −7.76 pcm/K in pcL's operating band; pcL uses a constant −10.000 pcm/K of undocumented
...
|---|---|---|
| 1 | Document or recompute pcL's −10 pcm/K Doppler coefficient | Case 5 — affects all transient conclusions |
| 2 | Generate pcM's 2-group MGXS (Phase 6); re-solve pcL's diffusion with it | Case 2/3 — isolates method error |
| 3 | Converge pcM's NekRS thermal field (~5 flow-throughs, ~5 h GPU) | Case 8 |
| 4 | Re-run pcM coupled production on ENDF/B-VIII.0 | Removes a +73 pcm library deviation |
...
| File | Contents |
|---|---|
| `results/pcm_openmc_unitcell.csv` | pcM k-infinity |
| `results/pcm_openmc_core.csv`, `_core_homog.csv` | pcM k-effective, heterogeneous and homogenised |
| `results/pcm_fuel_temperature_coefficient.csv` | pcM temperature coefficient sweep |
| `results/pcm_coupled_smoke.csv` | pcM coupled neutronics–conduction |
| `results/pcm_nekrs_convergence.csv` | pcM CFD convergence history |
| `results/reference_pcl_validation_step5.csv` | pcL lattice constants (copy) |
| `results/reference_pcl_validation_step6.csv` | pcL core and transient (copy) |
| `results/reference_pcl_SOURCE.md` | provenance of the pcL copies |
